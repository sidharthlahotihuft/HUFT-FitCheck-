...
returns AS MATERIALIZED (
    ...
    UNION ALL
    /* ONLINE returns. These are not in order_return_request at all — that
       table holds POS only. A web return is issued as a credit note, and
       sale_order_line_invoice_rel links the credit note line back to the
       exact order line it reverses.
       Note: credit notes carry no structured reason, so online returns have
       no 'size concern' tag. Direction still works, because that comes from
       what the customer bought next, not from a reason code.            */
    SELECT rel.order_line_id                    AS so_line_id,
           NULL::integer                        AS pos_order_line_id,
           am.invoice_date                      AS return_date,
           am.id                                AS return_request_id,
           'credit note'::text                  AS reason,
           ''::text                             AS reason_key
...
GROUP BY 1,2,3,4,5
HAVING count(*) >= 5
ORDER BY b.breed, b.channel, b.category_l1, b.category_l2, min(b.size_rank);
